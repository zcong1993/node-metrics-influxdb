...
Reporter.prototype.cronStart = function(cronTime, useBuffer, beforeReport = () => {}) {
  if (this.scheduleInterval) {
    console.warn('there already a report schedule running, would you want to use both of them ?')
  }
  let _beforeReport = () => {}
  if (beforeReport && typeof beforeReport === 'function') {
    _beforeReport = beforeReport
  }
  this._cronJob = new CronJob(cronTime, () => {
    _beforeReport()
    this.report(useBuffer)
  })
  this._cronJob.start()
}

Reporter.prototype.cronStop = function() {
  this._cronJob.stop()
  this._cronJob = null
}
